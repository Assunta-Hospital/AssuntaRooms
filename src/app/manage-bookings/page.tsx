...
export default function ManageBookingsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [{ data: bookingsData }, { data: usersData }, { data: roomsData }] = await Promise.all([
        supabase.from("bookings").select("*"),
        supabase.from("users").select("*"),
        supabase.from("rooms").select("*"),
      ]);

      if (bookingsData && usersData && roomsData) {
        const sortedBookings = [...bookingsData].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
