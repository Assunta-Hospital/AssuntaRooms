...
import { hospitalDepartments } from "@/lib/mock-data";
    ...
    <div className="flex items-center justify-center min-h-screen bg-background p-4">
      <Card className="mx-auto max-w-sm w-full shadow-lg">
        <CardHeader className="text-center space-y-4">
          ...
          <CardDescription>
            {mode === 'login' ? 'Enter your credentials to access the booking system' : 'Create an account to get started'}
          </Card-Description>
        </CardHeader>
        <CardContent>
          {mode === 'login' ? (
             <form onSubmit={handleLoginSubmit} className="grid gap-4">
                ...
                <div className="grid gap-2">
                    <div className="flex items-center">
                        ...
                        <Button variant="link" type="button" className="ml-auto inline-block h-auto p-0 text-sm">
                            Forgot password?
                        </Button>
                    </div>
            ...
            <form onSubmit={handleSignupSubmit} className="grid gap-4">
                <div className="flex flex-col items-center gap-4 mb-4">
                     <Label htmlFor="avatar-url" className="cursor-pointer">
                        <div className="w-24 h-24 rounded-full bg-muted flex flex-col items-center justify-center text-muted-foreground hover:bg-muted/80 transition-colors">
                           {avatarUrl ? (
                               <Image src={avatarUrl} alt="Avatar Preview" width={96} height={96} className="rounded-full object-cover w-full h-full" />
                           ) : (
                               <>
                    ...
                    <Input id="avatar-url" type="text" placeholder="Or paste image URL here" value={avatarUrl} onChange={(e) => setAvatarUrl(e.target.value)} disabled={isLoading} className="text-center" />
                </div>
                ...
                <div className="grid gap-2">
                    ...
                    <div className="relative">
                        <Input id="password-signup" type={showPassword ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)} disabled={isLoading} />
                         <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 flex items-center pr-3 text-muted-foreground">
                            {showPassword ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}
                        </button>
                    </div>
                 ...
                 <div className="grid gap-2">
                    <Label htmlFor="confirm-password-signup">Confirm Password</Label>
                     <Input id="confirm-password-signup" type={showPassword ? "text" : "password"} required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={isLoading} />
                </div>
            ...
            <div className="mt-6 text-center text-sm">
              {mode === 'login' ? (
                <>
                  ...
                  <Button variant="link" className="p-0 h-auto" onClick={() => { setMode('signup'); setShowPassword(false); }}>Sign up</Button>
                </>
              ) : (
                 <>
                  Already have an account?{" "}
                  <Button variant="link" className="p-0 h-auto" onClick={() => { setMode('login'); setShowPassword(false); }}>Sign in</Button>
                </>
              )}
            </div>
